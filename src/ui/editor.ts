...
import type { Guide, Rect, Step } from '../core/model.js';
...
class GuideEditor {
  ...
  private readonly status = element('span', 'save-state');
  private readonly title = element('h1');
  private reviewStatus: HTMLSelectElement | null = null;
  private readonly list = element('ol', 'step-list');
  private readonly stepCount = element('span');
  private readonly content = element('div');
  private readonly saveButton: HTMLButtonElement;
  private readonly fileInput = element('input');
  private paintNumber = 0;
  constructor(guide: Guide, private readonly main: HTMLElement) {
    this.guide = guide;
  ...
  private markDirty(resetStatus = true): void {
    this.dirty = true;
    if (resetStatus)
  ...
  private async mayDiscardImageEdits(): Promise<boolean> {
  ...
  private async selectStep(id: string): Promise<void> {
  ...
  private async applyImage(stepId: string, raster: Raster, annotations: Rect[]): Promise<void> {
    await this.save();
  ...
  private async duplicateStep(stepId: string): Promise<void> {
    if (!await this.mayDiscardImageEdits())
      return;
    await this.save();
    ...
    if (!original)
      return;
    const copy: Step = { ...original, id: crypto.randomUUID(), title: `${original.title.slice(0, 190)} (Kopie)`, imageId: null, annotations: original.annotations.map((rect) => ({ ...rect })) };
...
export async function renderEditor(main: HTMLElement, guideId: string): Promise<void> {
