...
export function hasWebsiteAccess(): Promise<boolean> {
  return chrome.permissions.contains({ origins: [ALL_WEBSITES] });
}
export async function requireWebsiteAccess(): Promise<void> {
...
export async function requestWebsiteAccess(): Promise<void> {
