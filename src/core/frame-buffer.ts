...
import type { Bounds, PreparedFrame, RecordedAction } from './model.js';
...
export function sameBounds(left: Bounds, right: Bounds): boolean {
  return ['x', 'y', 'width', 'height'].every((key) => {
    const coordinate = key as keyof Bounds;
...
export function frameMatchesAction(frame: PreparedFrame, guideId: string, action: RecordedAction): boolean {
...
export class FrameBuffer {
  private frames: PreparedFrame[] = [];
  add(frame: PreparedFrame): void {
  ...
  find(guideId: string, action: RecordedAction): PreparedFrame | undefined {
