...
import type { BackupEnvelope, Bounds, CaptureSnapshot, Guide, Rect, RecordedAction, Settings, Step, Viewport } from './model.js';
...
export function text(value: unknown, maximum: number, field = 'Text'): string {
...
export function parseAction(value: unknown): RecordedAction {
  const source = object(value);
...
export function parseSnapshot(value: unknown): CaptureSnapshot {
  const source = object(value);
...
export function settingsOrDefaults(value: unknown): Settings {
  try {
