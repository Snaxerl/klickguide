...
(() => {
    ...
    const fieldIntents = new WeakMap();
    ...
    function control(title, handler) {
    ...
    async function sendControl(command) {
        try {
            const reply = await chrome.runtime.sendMessage({ ...command, documentToken });
            if (!reply.ok)
    ...
    function receiveMutations(records) {
    ...
    function describe(element) {
        ...
        if (labelledBy) {
            ...
            const combined = labels.filter((item) => Boolean(item) && !isPrivate(item)).map(labelText).join(' ');
            if (combined.trim())
    ...
    function actionable(event) {
        if (!session || session.status !== 'recording' || !event.isTrusted || event.composedPath().includes(host))
            return undefined;
        return event.composedPath().find((item) => item instanceof Element && item.matches(controlSelector));
    }
    ...
    function actionFor(element, kind) {
        return {
    ...
    async function prepare(captureId) {
        ...
        try {
            await Promise.race([
                ...
                new Promise((_resolve, reject) => { paintTimeout = window.setTimeout(() => reject(new Error('Page did not paint')), 1200); }),
            ]);
            if (document.visibilityState !== 'visible')
    ...
    async function refreshFrame() {
        ...
        let retryAfter = 0;
        try {
            const reply = await chrome.runtime.sendMessage({ type: 'CACHE_FRAME', documentToken });
            if (reply.ok) {
    ...
    const onMessage = (value, sender, respond) => {
