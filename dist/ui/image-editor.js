...
export class ImageEditor {
    options;
    root = element('div', 'image-editor');
    canvas = element('canvas');
    controls = element('div', 'image-toolbar');
    hint = element('div', 'image-hint');
    applyButton;
    undoButton;
    cancelButton;
    toolButtons = new Map();
    ...
    constructor(options) {
        ...
        for (const [tool, title] of [['highlight', 'Markieren'], ['redact', 'Schwärzen'], ['crop', 'Zuschneiden']]) {
            const control = button(title, tool, () => { this.tool = tool; this.update(); }, 'ghost');
    ...
    setAccent(accent) { this.options.accent = accent; this.draw(); }
    get hasChanges() { return this.edits.length > 0; }
