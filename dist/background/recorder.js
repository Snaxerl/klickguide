...
function rememberDocument(session) {
    const now = Date.now();
...
export function sourceForAction(sender, action) {
...
async function connectRecorder(session) {
...
export async function startRecording(tabId, title) {
    ...
    const session = { guideId: guide.id, tabId, windowId: tab.windowId, origin: safeOrigin(tab.url ?? ''), status: 'recording', reason: '', stepCount: 0, lastCaptureAt: 0, documentToken: '', browserDocumentId: '', settings };
    try {
...
export async function pauseRecording(reason = 'Aufnahme pausiert.') {
...
export async function stopRecording(openEditor = true) {
    const session = await readSession();
    if (!session)
        return null;
    await saveSession(null);
...
async function verifyTarget(session) {
    await requireWebsiteAccess();
...
function documentAddress(session) {
    return session.browserDocumentId ? { documentId: session.browserDocumentId } : { frameId: 0 };
}
async function snapshotFrom(session, type, captureId) {
    const reply = await chrome.tabs.sendMessage(session.tabId, { type, captureId, documentToken: session.documentToken }, documentAddress(session));
    if (!reply.ok)
...
export async function cacheFrame() {
    const session = await readSession();
    const unavailable = { ready: false, stateRevision: -1, capturedAt: 0, retryAfter: 0 };
...
export async function recordAction(action, source) {
    const session = await readSession();
    ...
    const effectiveAction = action ?? { kind: 'manual', label: '' };
...
export async function onTabUpdated(tabId, change) {
    const session = await readSession();
