...
async function trustedRecorder(sender, command) {
...
async function handleMessage(value, sender) {
...
chrome.runtime.onMessage.addListener((message, sender, respond) => {
    void operations.enqueue(() => handleMessage(message, sender)).then((value) => respond({ ok: true, value }), (error) => {
...
function backgroundTask(task) {
    void operations.enqueue(task).catch((error) => console.error('KlickGuide background event failed', error));
}
